...
You can use examples/badger2040/image_converter/convert.py to convert 
them:

python3 convert.py --binary --resize image_file_1.png image_file_2.png 
image_file_3.png

Create a new "images" directory via Thonny, and upload the .bin files 
there.
"""
...
if TOTAL_IMAGES == 0:
    display.pen(15)
    display.clear()
    badger_os.warning(display, "To run this demo, create an /images 
directory on your device and upload some 1bit 296x128 pixel images.")
    time.sleep(4.0)
    sys.exit()
...
while True:
    ...
    if display.pressed(badger2040.BUTTON_A):
        ...
        changed = True
    if display.pressed(badger2040.BUTTON_B) or 
display.pressed(badger2040.BUTTON_C):
        display.pen(15)
        display.clear()
        badger_os.warning(display, "To add images connect Badger2040 to a 
PC, load up Thonny, and see readme.txt in images/")
        display.update()
    ...
    # Halt the Badger to save power, it will wake up if any of the front 
buttons are pressed
    display.halt()
