...
list_title = "Period Diary 1.0"
list_file = "Hellolist.txt"
...
try:
    with open("Hellolist.txt", "r") as f:
...
if save_checklist:
    with open("Hellolist.txt", "w") as f:
...
while True:
    ...
    if changed:
        badger_os.state_save("list", state)
# every change gets logged to the output file
        print(list_items,state)
        with open("output_data.txt", "w") as f:
            f.write(','.join(list_items) + "\n")
            f.write(str(state["checked"])[1:-1])

        with open("qrcodes/00_period_data.txt", "w") as f:
            f.write(str(state["checked"])[1:-1] + "\n")
            f.write("Period Data" + "\n")
            f.write("Scan This To ODK")
